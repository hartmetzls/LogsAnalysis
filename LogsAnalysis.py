...
def days_with_high_error_percentage():
    ...
    c.execute("CREATE VIEW dates_and_percentages AS "
              ...
              "dates_and_404s.errors / dates_and_statuses.stat_count ::float AS percentage "
              "FROM dates_and_404s JOIN dates_and_statuses "
              "ON dates_and_404s.date = dates_and_statuses.date;")
    c.execute("SELECT to_char(date, 'Month DD YYYY'), percentage from dates_and_percentages WHERE percentage > .01;")
    dates = c.fetchall()
    for a in dates:
